...
- 接口需要实现（implement），而抽象类只能被继承（extends）。由于Java只支持单继承，所以接口可以用于间接地实现多继承。
...
成员内部类可以自由地引用外部类的属性和方法，无论这些属性和方法是静态的还是非静态的。但但不能定义静态的属性和方法。它与一个实例绑定在一起，只有在外部类被实例化后，这个内部类才能被实例化。
