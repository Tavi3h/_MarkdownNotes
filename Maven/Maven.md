...
安装Maven前需要确保已安装JDK，且系统环境变量中已配置"JAVA_HOME"。
...
更新PATH变量，将apache-maven-3.5.3\bin目录添加到PATH变量中，以便于我们在任何目录中运行Maven命令。

配置完毕后，在cmd中执行`mvn -version`或`-v`：
